...
Item {
  ...
  // These three are `required property` upstream. They cannot stay required in
  // a cloned bar: shell.qml instantiates the built-in bar from an inline
  // Component that sets them at construction, but loads a plugin bar with
  // `Loader { source: <url> }` and only injects them from onLoaded — which is
  // after construction, so a required property is never initialized and the
  // Loader errors out with "Required property ... was not initialized". The
  // fallback in shell.qml then swallows the reason (`errorString` is not
  // defined in that scope) and silently reverts to the built-in bar.
  //
  // Giving them defaults makes construction legal; configureBar() still
  // assigns the real values on the very next tick.
  //
  ...
  component ModuleSlot: Item {
    ...
    // Tron: hovering a module lays a blurred, accent-tinted copy of it
    // underneath the real one, so the glyph reads as lit from inside instead
    // of sitting on a highlight pill. The three loaders below moved into
    // moduleContent only so there is a single item to sample; they still fill
    // the slot exactly as before.
    //
    // The copy goes through a ShaderEffectSource rather than moduleContent's
    // own layer: a MultiEffect consumes the layer it samples, so sourcing the
    // item directly renders the blur *instead of* the module and the bar loses
    // its sharp glyphs. ShaderEffectSource with hideSource false hands over a
    // texture and leaves the original on screen.
    //
    // live is false while unlit, so an idle bar is not re-rendering a texture
    // per module for a frame nobody is looking at.
    ShaderEffectSource {
      id: moduleTexture

      anchors.fill: moduleContent
      sourceItem: moduleContent
      hideSource: false
      live: moduleGlow.lit
      visible: false
    }

    // Two passes, because one blur of a 16px glyph spreads its light too thin
    // to read as a glow: a tight core that keeps the glyph shape, and a wide
    // halo bleeding into the bar around it.
    MultiEffect {
      id: moduleGlow

      readonly property bool lit: slot.hovered && !slot.dragSource

      anchors.fill: moduleContent
      source: moduleTexture
      z: -1
      visible: opacity > 0
      opacity: lit ? 1 : 0
      blurEnabled: true
    ...
    MultiEffect {
      ...
      source: moduleTexture
      z: -2
      visible: moduleGlow.visible
      opacity: moduleGlow.opacity
      blurEnabled: true
      blur: 1.0
      blurMax: 28
      blurMultiplier: 0.9
